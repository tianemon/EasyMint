...
.easymint/todos.json      用户待办（唯一清单源）：想法/计划，UI「待办」面板或文件读写维护
...
Mint 侧规则：
- **新增**：读 json → 追加条目（id = 现有最大 +1，保持结构完整）→ 写回；不要删除用户条目、不要改 title/note 原文
- **完成**：status 改 done + doneAt（UI 面板勾选同样操作）；用户面板下次打开会读到最新，无需广播
