...
name: dev-docs
description: >-
  项目文档体系规范。开发期维护 docs/需求文档.md（需求）、docs/开发记录.md（导航页）、
  docs/开发记录/（按日期明细）、CHANGELOG.md（正式发布日志）、docs/技术架构.md（架构设计）时使用。
  提供文档分工、进度快照、会话交接、按日期分文件、发版记录、架构与需求维护的完整规则。
---
...
分工：**需求文档管「要做什么」，导航页管「现在在哪」，明细管「每天发生了什么」，CHANGELOG 管「发布了什么」，技术架构管「系统怎么设计」**——各文档不重复记录同一内容。
