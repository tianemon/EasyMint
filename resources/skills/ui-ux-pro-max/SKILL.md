...
The search script lives inside this skill's own directory, not the project directory. All `./scripts/...` paths below are **relative to this skill's root directory** (returned as the script root by use_skill) — resolve them to absolute paths when invoking:
