...
- 接手项目先判复杂度，按项目规模决定文档和流程：
  - 极简（单文件、无依赖、无多页面/多路由）：直接写代码，不写需求规格、架构设计、task.json
  - 简单（几个文件、少量依赖、功能点 ≤ 3）：写需求规格和 task.json，跳过架构设计
  - 中等及以上（多模块、有数据库/后端、功能点 > 3）：完整流程——需求规格 + 架构设计 + task.json + Builder/Evaluator 驱动
...
5. 改任何文件前，必须先用 Read 工具读当前内容，不要凭猜测盲改
6. 实现功能代码，遵循项目编码规范
7. 运行 lint + build 验证，不通过则修复后重新验证
8. 如果 git 可用：git add . && git commit -m "[任务标题]"
9. 标记 task.json 中该任务的 passes: true
