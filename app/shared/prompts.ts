...
/*
 * .easymint/state.json 格式 — 由 Mint 在关键节点通过 project:writeState 写入，记录项目事实状态。
 * writeState 是合并写入（不会清掉已有字段），每次写只需包含要更新的字段。
 * refreshAll 优先读此文件判断阶段，无此字段时降级文件检测。
 * 不要写 stage 字段，只写事实。refreshAll 根据事实推导阶段。
{
  initCompleted: boolean,   // init.sh 是否执行成功
  docsLevel: "none" | "minimal" | "full",  // 已生成文档级别
  taskCount: number,        // task.json 总任务数
  doneCount: number,        // 已完成任务数
  lastSummary: string,      // 一句话：当前在做什么
  stageTimes: Record<string, number>  // 保留，不用改
}
*/
...
中断恢复：先读 task.json + docs/开发进度.md 确认进度，检查 escalation.json 优先汇报。
...
- docs/技术架构.md 中不需要后端/API/数据库相关章节
...
- docs/技术架构.md 需包含前后端架构、API 设计、数据库 schema
...
- docs/技术架构.md 中无需前端章节，关注模块划分和命令结构`,
...
- docs/技术架构.md 关注服务架构、中间件、数据库设计，无需前端章节`,
...
- docs/技术架构.md 关注组件树、状态管理、路由设计，无后端章节（除非有后端）
...
- 性能：列表懒加载、图片缓存、避免不必要的 rebuild`,
    evaluatorHint: `移动端项目：以代码审查为主——读实现代码对照需求文档逐项检查，运行 flutter test（如有），验证 build 通过。不截图`,
  },
...
- 测试：核心 API 需有单元测试覆盖`,
    evaluatorHint: `库/SDK 项目：运行 npm test 或等效测试，读实现代码对照需求文档，验证导出接口和 README 示例可用`,
  },
};
...
工作流程：
1. 读 docs/需求文档.md 了解项目背景和功能需求
2. 读 docs/技术架构.md 了解技术栈和系统结构
...
10. 标记 task.json 中该任务的 passes: true
