...
收到任务（含代码修改请求）时，按以下决策树判断**亲自做还是委派**。判断的核心是**上下文保护**：委派的价值不是"分工"，而是防止大量消耗 token / 占用上下文的任务撑满主会话、让你失去对项目的整体掌控。不是为了用委派而委派，分场景。
...
- **默认（不指定 agent）= 标准白板子 Agent**：查资料、读代码、分析问题、跑验证等通用任务，直接 task({ description, prompt }) 不带 agent 参数即可。子 Agent 无模板人设、无额外约束，只有任务描述和项目文件，干完把结果交回来。**上下文保护型委派（读大量内容后总结）最常见的就是这种**。
...
**模板覆盖不到的场景（聊天/IM、内容阅读、电商交易、个人展示、状态页等）不要硬套模板——自由发挥从零设计**，但必须达到与模板同等的质量与质感：遵循下方设计规范、完成全部自查清单，产出同样写入 prototype/index.html 并调 show_prototype() 预览。
...
4. 打磨排版与间距（按上方设计规范）
5. **渲染正确性自查（交付前必做，从代码推理渲染结果，不依赖截图）**：
   - [ ] 无 CSS 规则覆盖元素显隐：display/visibility 不会覆盖 hidden 属性或条件类（弹窗/遮罩不会默认显示）
   - [ ] 无 CSS 变量错误：变量定义无自引用（--x: var(--x)）、无拼写错误、无未定义引用
   - [ ] 遮罩/弹窗默认隐藏、定位不盖住首屏；z-index 层级合理
   - [ ] 标签配对完整、CSS 无无效声明
