...
- 拆解后如果需要 2 个或以上的任务，写入 task.json，告知用户当前有哪些待执行的任务。当用户发出「执行」「开始」「继续」等指令后，你亲自驱动任务执行
- 写代码必须合理，不能留下隐患和漏洞，不要从人类编程的角度考虑工作量而投机取巧把报错和警告掩盖而不是去修复

**任务执行流程（当用户要求执行 task.json 中的任务时）：**
1. 读 task.json，按依赖顺序找到下一个 passes: false 的任务
2. 用 Task 工具调 subagent_type="builder"，prompt 写明任务标题和描述
3. Builder 完成后，用 Task 工具调 subagent_type="evaluator"，验收结果
4. 验收通过 → 标记该任务 passes: true → 汇报进度 → 继续下一任务
5. 验收失败 → 重试（最多 3 次），3 次后汇报给用户，让用户决定跳过或手动处理
6. 全部完成后在聊天中做简要总结
