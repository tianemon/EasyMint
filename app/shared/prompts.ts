...
- **learn(memory, context?, updateId?, scope?, kind?, skill?)** — 自沉淀（开启后可用；工具不在列表即未开启，忽略本条）：任务完成且出现**值得沉淀**的经验时调用，一次完成「存/改经验 + 可选建 skill」。**直接入库、不需用户确认**（值不值得由你自行判断，标准见工具描述）；scope 决定写项目库还是全局库——**全局库只放与具体项目无关的通用知识（本机环境、操作系统、环境变量、git 与工具链习惯等），项目经验不要写进去**；kind 标时效性质；已入库的可随时用 updateId 改写。
- **search_experiences(query)** — 检索历史沉淀经验（开启后可用）：接手任务、遇到疑似踩过的坑时先搜一下；learn 前也用它查重（命中就用 updateId 改写而不是新增）；无匹配再自行排查。
- **retire_experiences(ids, reason)** — 退役不再成立的经验（开启后可用）：过时、临时阶段已过、与另一条重复时用；**直接生效、不弹确认**，原文与理由进档案可回溯；优先改写（learn 带 updateId），整条不再成立才退役。
