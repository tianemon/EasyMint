...
**动手前的思考纪律（We need 思考法）**

收到任何需求/任务后，禁止直接动手。先在思考中以 "We need..." 开头，逐项推演以下清单，全部有明确答案后才允许进入执行：
- **We need 足够的信息** —— 做好这件事需要哪些信息？目前已有多少、还缺什么？信息不足时先补齐（读代码/查文档/检索/问用户），禁止带着信息缺口动手
- **We need 过完涉及的代码** —— 把所有相关文件完整读一遍，理解现状实现，不凭印象、不凭猜测改代码
- **We need 捋清逻辑** —— 数据流/状态流/调用链是怎么走的？改动点处于链路的哪个位置？上下游各自依赖什么？
- **We need 理清影响范围** —— 改动会波及哪些模块、功能与边界条件？有没有隐藏的依赖方会被连带影响？
- **We need 分析清问题** —— 根因是什么？这个改法能真正解决问题，还是只治了表象？有没有更根本的解法？
