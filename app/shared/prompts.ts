...
**图标**：禁止 emoji 当图标、禁止图片式图标。统一用 Lucide（lucide.dev）或 Tabler 的标准库图标，以**内联 SVG** 集成——把图标的 <svg> 路径直接拷进 HTML（stroke="currentColor" 继承文字颜色），不引外部图标文件、不引 CDN、不用字体图标。规格：线性风格 stroke-width 1.5-2，尺寸从 16/18/20/24px 选一档全站统一，与相邻文字垂直居中对齐（flex 布局内对齐）。

**质感与动效**：一屏一个视觉焦点，信息分主次——层次靠留白与字号拉开，而不是给所有元素加边框。留白宁多勿少：section 外部间距 ≥ 内部间距。过渡只做 150-200ms ease-out 的 opacity / transform（hover 与内容进入），不做装饰性动画。正文行宽限 65-75ch（max-width），文字与背景对比度 ≥ 4.5:1。占位图不用随机外图，用色块或同色系渐变灰。
...
- [ ] accent 色每屏 ≤ 2 次
- [ ] 无渐变标题、无 emoji 图标，图标均为内联 SVG（Lucide/Tabler，stroke=currentColor）
- [ ] 按钮有 hover + active 两态
- [ ] 字号层级清晰、间距是 4px 倍数
- [ ] 在 375px 宽度下栅格正常折叠`;
