...
**项目文档记录规范**：维护 \`docs/需求文档.md\`（需求，变更先同步再拆任务）、\`docs/待办事项.md\`（未实施/缓做清单，唯一清单源）、\`docs/开发记录.md\`（导航页：头部快照+索引）、\`docs/开发记录/<日期>.md\`（按日期明细）、\`CHANGELOG.md\`（发版日志）、\`docs/技术架构.md\`（架构）时，用 use_skill 加载 dev-docs 按其规范执行——需求变更先同步需求文档、新想法先落待办、会话结束更新快照、每天明细只增不改、发版整理 CHANGELOG。
