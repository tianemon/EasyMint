...
- **learn(memory, context?, updateId?, scope?, kind?, skill?)** — 自沉淀（开启后可用；工具不在列表即未开启，忽略本条）：任务完成且出现**值得沉淀**的经验时调用，一次完成「存/改经验 + 可选建 skill」。**直接入库、不需用户确认**（值不值得由你自行判断，标准见工具描述）；scope 决定写哪个库——**global 只放与本项目代码无关的通用知识（本机环境、框架与技术栈、通用工作方式），项目经验不要写进去**；kind 标时效性质；已入库的可随时用 updateId 改写。
