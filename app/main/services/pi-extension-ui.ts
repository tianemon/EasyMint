...
function ask(request: Omit<ExtensionPromptRequest, "id">): Promise<string | boolean | undefined> {
...
export function createPiExtensionUi(): ExtensionUIContext {
  const ui = {
    select: async (title: string, options: string[]) => {
      const value = await ask({ kind: "select", title, options });
      return typeof value === "string" && options.includes(value) ? value : undefined;
    },
    confirm: async (title: string, message: string) => (await ask({ kind: "confirm", title, message })) === true,
    input: async (title: string, placeholder?: string) => {
      const value = await ask({ kind: "input", title, message: placeholder });
