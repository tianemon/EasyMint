...
const $env = { PI_TASK_MAX_OUTPUT_BYTES: "500000" };
const prompt = (s: string) => s;
const Snowflake = { next: () => String(Date.now()) };
const resolveAgentModelPatterns = (m: any) => m;
type LocalProtocolOptions = any;
const registerArtifactsDir = (dir: string) => {};
import { MCPManager } from "../../mcp/manager";
const loadOverallPlanReference = async () => "";
const planModeSubagentPrompt = "";
const subagentUserPromptTemplate = "";
const MAIN_AGENT_ID = "main";
type ToolSession = any;
const isIrcEnabled = () => false;
import { buildOutputValidator } from "../../tools/output-schema-validator";
const discoverAgents = async () => ({ agents: new Map() } as any);
const getAgent = (name: string) => undefined;
type DiscoveryResult = any;
type ExecutorOptions = any;
const runSubprocess = async (opts: any) => ({ results: [] as any[] });
import {
...
const generateTaskName = (t: string) => t.slice(0, 30);
...
type NestedRepoPatch = any;
const parseIsolationMode = (m: string) => undefined;
