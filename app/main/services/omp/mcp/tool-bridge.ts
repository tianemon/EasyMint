 ...
 */
type AgentToolUpdateCallback<T> = (result: T) => void;
type TSchema = object;
const normalizeSchemaForMCP = (s: unknown) => ({ schema: s }); // stub
const untilAborted = async (s: AbortSignal) => { if (s.aborted) return; await new Promise(r => { s.addEventListener("abort", r, { once: true }); }); };
const INTENT_FIELD = "__intent";
type SourceMeta = { name?: string; path?: string };
import type {
	CustomTool,
	CustomToolContext,
	CustomToolResult,
...
const resolveLocalUrlToFile = (u: string) => u;
type Theme = any;
