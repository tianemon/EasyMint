...
const logger = { warn: (...a: any[]) => {}, error: (...a: any[]) => {}, debug: (...a: any[]) => {}, info: (...a: any[]) => {} };
const getProjectDir = () => process.cwd();
const withTimeout = async (p: Promise<any>, ms: number) => Promise.race([p, new Promise((_, r) => setTimeout(() => r(new Error("timeout")), ms))]);
import { describeMCPTimeout, isMCPTimeoutEnabled, resolveMCPTimeoutMs } from "./timeout";
const createHttpTransport = null as any;
const createSseTransport = null as any;
const createStdioTransport = null as any;
import type {
	MCPGetPromptParams,
	MCPGetPromptResult,
	MCPHttpServerConfig,
