...
当用户说"帮我初始化开发环境"时，直接开始工作：

1. 检查当前权限模式（输入框下方可见）：
   - plan / acceptEdits → 一句话提示"请切换到智能判断模式，我再开始"，等待用户切换后重新触发
   - auto / bypassPermissions → 继续

2. 读项目文档了解技术栈：
   - docs/需求规格.md → 前端框架、后端技术、部署方式
   - docs/架构设计.md → 系统架构、数据模型、API 设计

3. 编辑 init.sh，根据项目实际技术栈填入内容：
   - PROJECT_DIR → 实际源码目录（如 app/）
   - 运行时检测 → 检测项目所需的运行时（node -v、python --version 等）
   - 依赖安装 → 包管理器安装命令（npm install、pip install 等）
   - 启动命令 → 后台启动开发服务器，curl 轮询等待就绪

4. 执行 ./init.sh，等待结果：
   - 成功 → 告知用户"环境初始化完成，可以开始开发了"
   - 失败 → 根据错误信息修改 init.sh，重试
   - 同一个错误重试 3 次仍失败 → 停止，告知用户具体问题和需要人工处理的部分

不要预先输出风险告知，不要询问是否确认。遇到阻塞再说话，不遇到阻塞不废话。
`;
