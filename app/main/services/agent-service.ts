...
async function createAskUserTool(sessionId: string): Promise<ToolDefinition> {
  ...
  return defineTool({
    ...
    promptGuidelines: [
      "先查证再判断：只对仍不明确的关键意图、未授权的视觉/交互/业务行为或范围/成本取舍提问；目标明确且已授权的任务、纯正面收益修复及内部技术选型直接推进，不问是否开始/继续",
      "仅存在多种技术实现不触发提问；修法会改变用户可感知行为或引入真实取舍才问。提问前给推荐理由，说明改/不改的预期效果与代价；选项互斥，级联问题用 depends_on 关联",
      "调用后回合暂停等待用户回答，回答会作为工具结果返回，据此继续推进",
      "用户跳过或取消不等于授权；待决定部分保持现状，继续不依赖该决定的已授权工作，不重复催问",
...
判定为不值得沉淀时：**静默跳过**——不调 learn、也不要在回复里提及沉淀，继续正常汇报即可。值得沉淀就调 learn **直接入库（无确认环节）**；入库后仍可随时用 updateId 改写、或用 retire_experiences 退役——沉淀与清理都由你自行判断。`,
   ...
   * **唯一落盘入口——用前先自问：这份内容在会话里还有别的载体吗？**
   *   有（如摘要已在 compaction 条目里）→ 不该走这里：去让显示层渲染那条已有记录，
   *   否则同一份内容会存两遍，每轮请求都多带一遍（用户明确要求：不重复落盘）。
   *   没有（委派/后台命令结果只以事件形式存在）→ 才注入。
