...
interface ActiveChat {
  ...
  /** Skip the next user message from stream — set before internal ops like setModel */
  skipNextUserMsg: boolean;
}
    ...
    (async () => {
      try {
        ...
        for await (const msg of queryObj) {
          ...
          if (event) {
            // Suppress messages generated by internal operations (e.g. setModel)
            if (chat.skipNextUserMsg) {
              chat.skipNextUserMsg = false;
              continue;
            }
