...
import { droppedContextIds, imageStrippedContextIds } from "./context-edit";
...
async function parseEntriesToMessages(mgr: { getEntries(): unknown[]; getLeafId(): string | null }, sessionId: string, includeToolResult = false): Promise<SessionMessage[]> {
  ...
  const imageStripped = imageStrippedContextIds(entries);
  ...
  for (const entry of entries) {
    if (entry.type === "message") {
      ...
      messages.push({
        ...
        ...(imageStripped.has(entry.id) ? { image_stripped: true } : {}),
      });
