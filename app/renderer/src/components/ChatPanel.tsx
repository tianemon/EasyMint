...
const MemoChatMessage = memo(function MemoChatMessage({ msg, busy, userBubble, onPin, onContextMenu, sid }: MemoChatMessageProps) {
  ...
  if (msg.role === "user") {
    ...
    if (kind) {
      ...
      const first = rows[0]?.match(/^⏺ (.+?) [-—] (完成|失败|中止|已由用户中断|已由用户停止|已终止)(?: · (\d+)s)?$/);
      ...
      // 中止/已由用户中断/已由用户停止/已终止=人为主动停止(黄),失败=意外中断(红),完成=绿
      const statusColor = (s?: string): string =>
        (s === "中止" || s === "已由用户中断" || s === "已由用户停止" || s === "已终止") ? "text-interrupt" : s === "失败" ? "text-fail" : s ? "text-done" : "";
                ...
                {isResult ? (
                  <div className="overflow-y-auto overscroll-contain" style={{ maxHeight: "calc(var(--text-detail) * 9.75 + 12px)" }}>
                    {lines.map((row, i) => {
                      ...
                      const m = row.match(/^⏺ (.+?) [-—] (完成|失败|中止|已由用户中断|已由用户停止|已终止)(?: · (\d+)s)?$/);
