...
- 验证自动更新链路：从 0.3.2 升级到本版本

## v0.3.2 (2026-06-26)

### 自动更新
- 接入 electron-updater，对接 GitHub Releases 实现应用内自动更新
- 启动后 10s 首次检测，之后每 4 小时自动检测一次新版本
- 检测到新版本后台自动下载（Windows 走 blockmap 差量、macOS 整包替换）
- 下载完成后左下角设置按钮显示红点，点击弹出上拉菜单「重启更新到 vX.X.X」
- 已下载的更新在应用退出时自动安装

### 关于页优化
- 版本号改为动态读取（修复一直显示 v0.1.0 的问题）
- 版本号旁加刷新图标，支持手动检测更新
- 显示更新状态：检查中 / 发现新版本下载中 / 下载进度 / 已就绪可安装 / 已是最新

### 多窗口
- 更新状态广播给所有窗口，多窗口场景红点同步
