...
### 问题记录
- 新增 Issue 面板（右侧侧边栏 I 图标），记录项目问题
- 表单：标题、功能模块、问题现象，引导小白用户描述准确
- 状态：未修复 / 已修复 手动切换，删除二次确认
- 追加功能：后续补充新现象，每次带时间戳独立记录
- Mint 一键分析：对话中说「看一下记录的问题」，list_issues MCP 工具读取所有 issue
- 文件存 `<project>/.easymint/issues.json`，纯本地

### 运行进程管理
- 新增运行面板（右侧侧边栏 R 图标），一键启动/停止/重启项目进程
- 启动配置由 Mint 开发完生成 `.easymint/run.json`（非手动扫描）
- 支持多命令独立启停、Overlay 日志弹窗（关闭不停止）
- 日志纯内存、停止销毁，不写本地
- 平台前缀着色（22 种框架），install_command 安装依赖按钮，url 启动后打开

### 修复
- macOS 合盖唤醒聊天 tab 丢失：主进程备份 tab 状态，渲染进程重建后恢复
- 环境检测：npx/CodeGraph node 脚本 PATH 补全、Windows 跨平台路径
- 缓存管理：整合更新缓存和上传缓存，扫描大小 + 清除按钮
- NSIS 卸载不再删除用户数据

### 优化
- 会议列表去掉日期分组，平铺显示，当天显示「今天 hh:mm」
- 设置去掉外观主题区、Agent 模板页面
- Vite/esbuild 构建警告消隐
- Mint 提示词补充 run.json 生成规范
