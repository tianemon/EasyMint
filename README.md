...
- **不把数据放在云上**——项目文件与会话记录都留在本机（`~/.easymint/` 与项目内 `.easymint/`），换设备用局域网直传（加密 + 整包校验），不经过任何服务器；**没有账号、没有遥测**。只有你发起对话时，对话内容（提示词、项目上下文、代码与命令输出、图片）才会发给你自己配置的 AI 供应商——范围与边界见 [PRIVACY.md](PRIVACY.md)
...
- **Pi 扩展互通**——扫描原生 Pi 的用户级与项目级扩展，在设置中确认启用后供完全访问模式的新会话使用；原生配置与安装文件保持原位
...
- **数据主权**——项目文件与会话数据存储在本地（`~/.easymint/` 与项目内 `.easymint/`），不上云、不锁定；**发什么给模型供应商、Agent 能做什么、凭据存在哪，逐条写在 [PRIVACY.md](PRIVACY.md)**
...
Pi 扩展是用户明确授权的主进程代码，仅在**完全访问模式**的会话中加载。扩展初始化与事件处理不经过普通工具的内核沙盒；切回标准或只读后，当前回答可完成，扩展工具立即停用，会话在空闲时关闭并按新权限重建。
...
**数据边界是另一件事**——上面三档管的是「Agent 能做什么」，不等于「什么内容会离开本机」：对话内容、Agent 为干活而读到的文件与命令输出、注入的项目上下文、以及图片，都会由本机**直连**发送给你自己配置的 AI 供应商（该供应商的隐私与数据保留政策随之适用）；联网搜索与抓取另经 Tavily。完整清单、遥测说明与凭据存储方式见 [PRIVACY.md](PRIVACY.md)。
...
- **API Key**——其余内置供应商（**OpenAI**、DeepSeek、智谱 GLM / Z.AI、MiniMax、Qwen、Google Gemini、小米 MiMo、OpenCode 等）与自定义供应商；密钥与账号凭据一样只存在本机，**以明文存放于 `~/.easymint/`**（与 `.npmrc`、`git-credentials`、`~/.aws/credentials` 同类做法；MCP 的 OAuth 凭据则走系统钥匙串加密）——详见 [PRIVACY.md](PRIVACY.md)
