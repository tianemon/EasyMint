...
**try/catch 只用于有明确业务逻辑需要处理的场景**。原则：

1. catch 必须有实质性业务逻辑——给前端返回提示、走备用路径、回退到默认值——不接受空 catch 或只记日志
2. 只在"预期内可能发生"的错误上 catch：SDK 网络调用超时/失败、用户手动修改的配置文件 JSON 损坏、扫描外部目录时的权限问题
3. **本地文件操作不加 try/catch**——读自己的配置文件、写日志、require 本地模块等，这些挂了说明环境坏了或构建坏了，应该让错误暴露出来
4. 不需要 catch 来"保启动不崩""保对话不炸"——坏了就该暴露，不该偷偷吞掉
5. catch 里只 throw 同一个 err 等于没 catch，直接删掉 try/catch

检验标准：如果 catch 块删掉后程序行为完全不变（除了不再吞错误），那这个 try/catch 就是多余的。
