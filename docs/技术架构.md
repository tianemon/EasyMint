...
### projects.json (`~/.easymint_pi_core/`)
...
### em-settings.json (`~/.easymint_pi_core/`)
...
Skills 通过 Pi SDK 的 `resourceLoader` 加载，扫描 `~/.easymint_pi_core/skills/` 目录，注入 system prompt。
...
`hooks.ts` 中的 `validateTaskStatus` / `validateProjectDone` 在 MCP 工具 `execute` 内部做状态一致性校验（不依赖 SDK hook 机制）：
...
```
会话 JSONL: ~/.easymint_pi_core/sessions/<编码路径>/<sessionId>.jsonl
